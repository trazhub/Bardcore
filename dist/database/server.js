...
export default class ServerData {
    ...
    setPrefix(guildId, prefix) {
        const data = db.prepare('SELECT * FROM guild WHERE guildId = ?').get(guildId);
        if (!data) {
    ...
    delete_247(guildId) {
        const data = db.prepare('SELECT * FROM stay WHERE guildId = ?').get(guildId);
        if (!data)
            return;
    ...
    getRoles(guildId) {
        const data = db.prepare('SELECT * FROM roles WHERE guildId = ?').all(guildId);
        if (!data) {
            return false;
        }
        else {
    ...
    addRole(guildId, roleId) {
        const data = db.prepare('SELECT * FROM roles WHERE guildId = ? AND roleId = ?').get(guildId, roleId);
        if (!data) {
    ...
    getBotChannel(guildId) {
        const data = db.prepare('SELECT * FROM botchannel WHERE guildId = ?').get(guildId);
        if (!data) {
    ...
    getSetup(guildId) {
        const data = db.prepare('SELECT * FROM setup WHERE guildId = ?').get(guildId);
        if (!data) {
    ...
    getPLaylist(userId, name) {
        const data = db.prepare('SELECT * FROM playlist WHERE userId = ? AND name = ?').get(userId, name);
        if (!data) {
            return false;
        }
        else {
    ...
    removeSong(userId, name, song) {
        const data = db.prepare('SELECT * FROM playlist WHERE userId = ? AND name = ?').get(userId, name);
        if (data) {
            db.prepare('UPDATE playlist SET songs = ? WHERE userId = ? AND name = ?').run(JSON.stringify(data.songs.filter((s) => s !== song)), userId, name);
        }
        else {
            throw new Error('Playlist does not exist');
        }
